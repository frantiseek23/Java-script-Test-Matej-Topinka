...
function processArrayItems(arrayInput, minLength, maxLength, symbolToFill) {
 ...
 for (const item of arrayInput) { if (item.length >= minLength && item.length <= maxLength)
 { const filledItem = item.length < maxLength ? item + symbolToFill.repeat(maxLength - item.length) : item;
